...
const calculateTotal = (items) => {
  ...
  const total = items.reduce((sum, item) => {
    ...
    console.log(
      `項目: ${
        item.name || item.id
      }, 價格: ${validatedPrice}, 數量: ${validatedQuantity}, 小計: ${itemTotal}`
    );
  ...
  try {
    ...
    if (error.response) {
      switch (error.response.status) {
        ...
        case 401:
          throw new Error(
            "結帳失敗：您的帳戶目前可能沒有完成購買所需的權限，或者您的登入狀態需要刷新。請嘗試完全登出後再重新登入。如果問題持續，請聯繫客服。"
          );
        case 403:
