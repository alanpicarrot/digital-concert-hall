...
import { Loader2, CreditCard, ShoppingBag, Calendar, ArrowLeft, ShieldCheck, Info, Receipt } from 'lucide-react';
        ...
        if (orderNumber) {
          ...
          if (checkoutInfoStr) {
            try {
              ...
              // 檢查是否有折扣
              if (checkoutInfo.discountCode || checkoutInfo.discountAmount > 0) {
    ...
    try {
      ...
      if (directCheckout) {
        try {
          ...
          // 添加日誌以確認數據
          if (process.env.NODE_ENV === 'development') {
            console.log('創建直接購買訂單:', JSON.stringify(cartRequest, null, 2));
          }
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-100">
                    ...
                    {showReceiptPreview && (
                      <div className="mt-3 border border-dashed border-gray-300 rounded p-3 bg-gray-50">
                        <h4 className="font-medium text-sm mb-2">電子收據預覽</h4>
                        <div className="text-xs text-gray-600 space-y-1">
                          <p>購買者：{localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')).name : '未登入用戶'}</p>
                          <p>商品：數位音樂廳票券</p>
              ...
              <div className="p-4">
                <div className="flex items-center border rounded p-3 mb-4 bg-gray-50">
                  ...
                  <label htmlFor="credit-card" className="flex items-center justify-between w-full">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">安全、快速的付款方式</p>
                    </div>
                ...
                <div className="flex items-center border rounded p-3 mb-4 bg-gray-50 opacity-50 cursor-not-allowed">
                  <input
                    ...
                    disabled
                  />
                  <label htmlFor="atm-transfer" className="flex items-center justify-between w-full">
                    <div>
                      <span className="font-medium text-gray-500">ATM 轉帳付款</span>
                ...
                <div className="bg-indigo-50 rounded p-3 border border-indigo-100 text-xs text-indigo-700">
                  <p className="mb-1">點擊「確認付款」後，系統將導引您至綠界金流進行安全支付。所有支付信息均使用SSL加密傳輸。</p>
                  <p>完成付款後，您將立即收到電子票券與收據到您的電子信箱，也可以在「我的訂單」中查看。</p>
                </div>
              </div>
            </div>
