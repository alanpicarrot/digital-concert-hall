...
const PaymentStep1 = () => {
  ...
  useEffect(() => {
    ...
    if (orderNumber) {
      ...
      // 如果沒有URL參數，嘗試從sessionStorage中獲取
      const storedDetails = sessionStorage.getItem('checkoutInfo');
      if (storedDetails) {
        try {
          const parsedDetails = JSON.parse(storedDetails);
          setOrderDetails({
            orderNumber: parsedDetails.orderNumber || 'ORD' + Date.now().toString().slice(-8),
            productName: parsedDetails.concertTitle + ' x ' + parsedDetails.quantity || 'VIP票 x 1',
            amount: parsedDetails.totalAmount || 2000
          });
        } catch (err) {
  ...
  const handleContinue = () => {
    navigate(`/payment/steps/payment?orderNumber=${orderDetails.orderNumber}&amount=${orderDetails.amount}&productName=${encodeURIComponent(orderDetails.productName)}`);
  };
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-center">
              <div className="flex items-center">
                <div className="bg-indigo-600 text-white w-8 h-8 rounded-full flex items-center justify-center">1</div>
            ...
            <div className="p-5">
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                  ...
                  <span className="font-bold text-xl text-indigo-600">NT$ {orderDetails.amount}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-8">
            <div className="p-5 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-800">選擇支付方式</h2>
            </div>
            <div className="p-5">
              {/* 支付選項 */}
              <div className="space-y-3">
                <div className="border rounded-lg p-4 bg-indigo-50 border-indigo-200">
                  <label className="flex items-center">
                    <input type="radio" name="payment-method" className="form-radio text-indigo-600" defaultChecked />
                    <div className="ml-3 flex-grow">
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="font-medium text-gray-800">信用卡支付</span>
                          <p className="text-sm text-gray-500 mt-1">使用Visa、MasterCard、JCB卡片支付</p>
                        </div>
                ...
                <div className="border rounded-lg p-4 border-gray-200">
                  <label className="flex items-center opacity-50 cursor-not-allowed">
                    <input type="radio" name="payment-method" className="form-radio text-gray-400" disabled />
                    <div className="ml-3">
                      <span className="font-medium text-gray-600">Line Pay</span>
                      <p className="text-sm text-gray-500 mt-1">測試環境暫不支援</p>
                    </div>
                  </label>
                </div>
              </div>
