...
const UsersPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axiosInstance.get(validateApiPath('/api/admin/users'));
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {error && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start">
          <AlertTriangle className="text-red-500 mr-2 flex-shrink-0 mt-1" size={20} />
          <div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-wrap gap-1">
                        {user.roles && user.roles.map((role, index) => (
                          <span
                            key={index}
                            className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getRoleBadgeClass(role)}`}
                          >
                            {formatRoleName(role)}
                          </span>
                        ))}
                ...
                <div>
                  <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <div className="flex items-start">
                      <AlertTriangle className="text-amber-500 mr-2 flex-shrink-0 mt-1" size={20} />
                      <div>
                        <p className="text-amber-700 font-medium">確定要刪除此用戶嗎？</p>
                        <p className="text-amber-600">此操作無法撤銷，用戶的所有資料將被永久刪除。</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center">
                        <input
                          ...
                          defaultChecked={selectedUser?.roles?.includes('ROLE_ADMIN')}
                        />
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          id="role-moderator"
                          className="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded"
                          defaultChecked={selectedUser?.roles?.includes('ROLE_MODERATOR')}
                        />
                        <label htmlFor="role-moderator" className="ml-2 text-gray-700">
                          版主
                        </label>
                      </div>
                      <div className="flex items-center">
                        <input
                          ...
                          id="role-user"
                          className="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded"
                          defaultChecked={selectedUser?.roles?.includes('ROLE_USER')}
                        />
